...
class FB15K():
	...
	def __init__(self, load_dataset = None, embedding_model = None, embedding_size = None, margin = None, learning_rate = None, patience = None, batch_size = None, sampling_type = None, save_dataset = None):
		...
		if not load_dataset:
			...
			self.entity2id = {entity:int(i) for line in open("./Datasets/FB15K/entity2id.txt").readlines()[1:] for entity, i in [line.strip().split('\t')]}
			self.id2entity = {self.entity2id[entity]:entity for entity in self.entity2id}

			self.relation2id = {relation:int(i) for line in open("./Datasets/FB15K/relation2id.txt").readlines()[1:] for relation, i in [line.strip().split('\t')]}
			self.id2relation = {self.relation2id[relation]:relation for relation in self.relation2id}

			self.entity_count = len(self.entity2id)
			self.relation_count = len(self.relation2id)

			self.triplets = numpy.array([[int(h), int(r), int(t)] for line in open("./Datasets/FB15K/train2id.txt").readlines()[1:] for h, t, r in [line.strip().split(' ')]])
...
if __name__ == "__main__":
	...
	with tensorflow.Session() as session:
		...
		FB15K(
			...
			save_dataset = "Dataset_FB15K_MAIN"
		)
